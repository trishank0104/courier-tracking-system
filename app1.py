...
jwt = JWTManager(app1)
CORS(app1, resources={r"/*": {"origins": ["http://your-frontend-domain.com"]}})  # Restrict origins in production
...
def login():
    try:
        ...
        cursor.close()
        if not user or not bcrypt.checkpw(password.encode('utf-8'), stored_password.encode('utf-8')):
            return jsonify({"error": "Invalid credentials"}), 401
        # Use user_id as string identity and add role as additional claim
...
def register():
    try:
        ...
        data = request.json
        name = data.get('name')  # Used for delivery_person, username for admin
        email = data.get('email')
        password = data.get('password')
        role = data.get('role')  # Expected: 'admin' or 'delivery_person'
        if not all([name, email, password, role]):
            return jsonify({"error": "Missing required fields"}), 400
        if role not in ['admin', 'delivery_person']:
            return jsonify({"error": "Invalid role specified"}), 400
        cursor = mysql.connection.cursor()
        if role == 'admin':
